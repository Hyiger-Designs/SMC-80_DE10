...
-- Command Line : ./fracn09.pl -e fracn_1 50000000 1
-- Date         : Thu May 28 00:05:30 2020
...
-- Desired Output Frequency: 1 Hz.
-- Requested Relative Frequency Error Bounds (+/-) : 1e-07 (0.1 ppm)
--
-- Frequency Results (use_phase_accumulator = FALSE) :
--  Achieved Output Frequency: 0.99999990000001 Hz.
--  Achieved Relative Frequency Error: -9.99999899553572e-08 (-0.0999999899553572 ppm)
--  Achieved Frequency Error: -9.99999899553572e-08 Hz.
--
-- Frequency Results (use_phase_accumulator = TRUE) :
--  Achieved Output Frequency: 1.00000008274037 Hz.
--  Achieved Relative Frequency Error: 8.27403709990904e-08 (0.0827403709990904 ppm)
--  Achieved Frequency Error: 8.27403709990904e-08 Hz.
--
-- Output Jitter Parameters (use_phase_accumulator = FALSE) :
--  The fundamental frequency is 0.99999990000001 Hz.
...
--  The fundamental frequency is 7.105427357601e-07 Hz.
--  The amplitude is 2e-08 seconds p-p (approx).
--
-- Design Parameters (use_phase_accumulator = FALSE) :
--  Approx 29 flip flops (26 in prescaler, 1 in controller and 2 retimes).
--  The recursive controller uses approx 1 flip flop.
--  The Dual-Modulus Prescaler uses ratios /50000005,/50000006
--  The Output consists of 1 cycle of 50000005 input clocks,
--  and 0 cycles of 50000006 input clocks.
--  There is 1 output clock for every 50000005 input clocks.
--
-- Design Parameters (use_phase_accumulator = TRUE) :
--  Approx 48 flip flops (46 in accumulator and 2 retimes)
--  There are 1407375 output clocks for every 70368744177664 input clocks.
--
-- Divider summary :
--
...
--
--  48    tbd       8.3e-08   2e-08     use_phase_accumulator
--  51    tbd       8.3e-08   1e-08     use_phase_accumulator improve_duty_cycle
--  29    tbd       -1e-07    0         use_recursive_controller
--  30    tbd       -1e-07    0         use_recursive_controller improve_duty_cycle
--  29    tbd       -1e-07    0         minimum_jitter
--  30    tbd       -1e-07    0         minimum_jitter improve_duty_cycle
--  29    tbd       -1e-07    0         (none)
--  30    tbd       -1e-07    0         (none) improve_duty_cycle
--
-- Warnings:
--* Warning: a fractional-N divider is not needed.
--*   Integer dividers 49999996 to 50000005 are ok.
--
...
entity fracn_1 is
    ...
    port (
        ...
        output_50         : out std_logic;  -- 1 Hz output - approx 50% duty cycle
        output_pulse      : out std_logic   -- 1 Hz output - high for single clock per cycle
    );
end fracn_1;
...
architecture rtl of fracn_1 is
    ...
    constant n              : positive := 50000005; -- prescaler divides by n or n + 1
    constant a              : positive := 1;    -- this many counts of 50000005
    constant b              : natural  := 0;    -- this many counts of 50000006
    ...
    constant num_bits       : positive := 46;   -- size of phase accumulator
    constant c              : unsigned(num_bits - 1 downto 0) := to_unsigned(1407375, num_bits);
...
--------------------------------------------------------------------------------
-- Prescaler.  Divides by either 50000005 or 50000006
...
--          n0=50000005 m0=X        a0=1        b0=0        i0=X
